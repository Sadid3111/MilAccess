...
class MilitaryTask {
  ...
  factory MilitaryTask.fromMap(Map<String, dynamic> task) {
    return MilitaryTask(
      id: task['id'],
      task: task['task'],
...
class _ToDoListScreenState extends State<ToDoListScreen>
  ...
  Future<void> fetchTasks() async {
    try {
      ...
      docRef.docs.forEach((doc) {
        final task = doc.data();
        _militaryTasks.add(MilitaryTask.fromMap(task));
      });
    } catch (e) {
      if (!mounted) return;
  ...
  Future<void> _toggleTaskCompletion(MilitaryTask task) async {
    setState(() {
  ...
  int get _pendingCount =>
      _militaryTasks.where((task) => !task.isCompleted).length;
  int get _completedCount =>
      _militaryTasks.where((task) => task.isCompleted).length;
  int get _overdueCount =>
      _militaryTasks.where((task) => task.isOverdue).length;
  int get _highPriorityCount => _militaryTasks
